function rockfish_mockdata(Nsim,Meta,F1,F2,T1,T2,BinW,savename,savedir,Fcv,Rsd,Rcorr)
...
% Nsim is the number of simulated datasets to create
% Meta is the file where the relevant metadata is located
% F1 is the 'pre-data' value of F
% F2 is the value of F during data collection, the value to be estimated
% F1 and F2 can be vectors to allow F to vary with time
% T1 & T2 are the pre-data and during-data time spans, respectively
% BinW is the width of length bins, allowing simulation of wider binning of
% data than 1 cm
% savename is the file name to be saved
% savedir is the directory for saving
% Fcv is the cv of F (optional)
...
if ~exist('Fcv','var'); Fcv = 0; end
if ~exist('Rsd','var'); Rsd = 0.1; end
if ~exist('Rcorr','var'); Rcorr = 0; end
...
Rmean = 50;
%Rsd = 0.1;
if Rcorr > 0 % if we need an autocorrelation matrix
    Lags = repmat(1:T,[T,1]);
    Lagmat = Lags - Lags';
    Corrmat = Rcorr .^ abs(Lagmat); % correlation matrix
    Covmat = corr2cov(repmat(Rsd,[T,1]),Corrmat);
...
R = exp( mvnrnd( repmat(log(Rmean),[T,1]),Covmat) );
end
...
% Fishing mortality rate + process error
Fvec = [ones(1,T1)*F1, normrnd(F2,F2*Fcv,1,T2)];
