...
for t = 2:length(T)
   ...
   % Advance the model, one particle at a time
    ...
    % avoid zero values. constrain to the smallest nonzero value.
    % poisspdf declines linearly with lambda for values below one, so using
    % a very low floor like realmin causes issues.
    ZeroInd = Nf(:,t,q)<=0;
    ZeroMin = min(Nf(~ZeroInd,t,q));
    Nf(ZeroInd,t,q) = ZeroMin;
