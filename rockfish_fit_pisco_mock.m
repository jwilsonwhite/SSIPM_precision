function rockfish_fit_pisco_mock(Meta,pauseflag,SS)
...
% Adjust samplesize (if this option is called for):
if ~isnan(SS)
...
Ogive_b = Meta.ogive;
Ogive_b = [NaN, NaN]; % turn this feature off for mock data
...
for c = 1:chains
    ...
    Ltmp(s) = do_IPM(meshsize,x,dy,Sy,parm_vec_tmp,fixparm,Rfact,R,squeeze(N(:,s,:)),T,Tpre,Ogive,Meta,NT(s,:),Site_Names{s});
    ...
    PriorL(2) = log(max(realmin,normpdf(log(parm_vec_tmp(2)),log(Meta.Fprior(2,1)),Meta.Fprior(2,2)))); % F2
        case 'normal'
    PriorL(1) = log(max(realmin,normpdf((parm_vec_tmp(1)),(Meta.Fprior(1,1)),Meta.Fprior(1,2)))); % F1
    PriorL(2) = log(max(realmin,normpdf((parm_vec_tmp(2)),(Meta.Fprior(2,1)),Meta.Fprior(2,2)))); % F2
    end
...
for s = 1:length(Site_Names)
    keyboard
[L1(s), Npred,Nact] = do_IPM(meshsize,x,dy,Sy,mean(parm_vec(1:end,:)),fixparm,Rfact,R,squeeze(N(:,s,:)),T,Tpre,Ogive,Meta,NT(s,:),Site_Names{s},1);
