function analyze_vary_F(do_postproc)
...
for s = 1:length(Species)
    ...
    for f = 1:length(Fmu)
        ...
        for j = 1:length(Nsims)
            ...
            Meta = M.Meta;
            Meta.data_savename = strcat('mockdata/',Species{s},'_mockdata_F',num2str(Fmu(f)),'_Fcv',num2str(Fcv),'_',Date,'_R',num2str(Nsims(j)),'.mat');
            Meta.fit_savename = strcat('mockfits/',Species{s},'_Mock_fit_F',num2str(Fmu(f)),'_Fcv',num2str(Fcv),'_',Date,'_R',num2str(Nsims(j)),'.mat');
    ...
    Bias = nan(length(Species)*length(Fmu),1);
    StDev = Bias;
    Fs = Bias;
    Specieslist = cell(length(Species)*length(Fmu),1);
    ...
    for f = 1:length(Fmu)
        ...
        Index = (s-1)*length(Fmu)+f; % the row of Dataframe to use

        % do post-processing
        for j = 1:length(Nsims)
            M = load(meta_fname,'Meta');
            Meta = M.Meta;
            Meta.postproc_savename = strcat('mockfits/',Species{s},'_Mock_fit_F',num2str(Fmu(f)),'_Fcv',num2str(Fcv),'_',Date,'_R',num2str(Nsims(j)),'_postproc.mat');
...
Dataframe = table(Specieslist,Fs,Bias,StDev);
keyboard

writetable(Dataframe,'Fvary_bias.csv')
